...
func main() {
	...
	dbPath := flag.String("db-path", filepath.Join(os.Getenv("HOME"), ".cache", "klipb", "db"), "path to db")
	configPath := flag.String("config-path", "$XDG_CONFIG_HOME/klipb/config", "overwrite config path to use instead of cli flags")
...
func store(dbPath string, in io.Reader, maxDedupeSearch, maxItems uint64) error {
	...
	countBucket := tx.Bucket([]byte(countsBucketKey)) // Access counts bucket
	...
	countKey := itob(id)
	count := btoi(countBucket.Get(countKey)) // Get the current count
	count++
	if err := countBucket.Put(countKey, itob(count)); err != nil {
...
func wipe(dbPath string) error {
	...
	// Reset the counts bucket
	countBucket := tx.Bucket([]byte(countsBucketKey))
	countCursor := countBucket.Cursor()
	for k, _ := countCursor.First(); k != nil; k, _ = countCursor.Next() {
		_ = countBucket.Delete(k) // Clear each count entry
	}
...
func initDB(path string) (*bolt.DB, error) {
	return initDBOption(path, false)
}

func initDBReadOnly(path string) (*bolt.DB, error) {
	return initDBOption(path, true)
}
...
func initDBOption(path string, ro bool) (*bolt.DB, error) {
	...
	err = db.Update(func(tx *bolt.Tx) error {
		...
		_, err = tx.CreateBucketIfNotExists([]byte(countsBucketKey)) // Create counts bucket
		return err
	})
	if err != nil {
...
func deduplicate(b *bolt.Bucket, input []byte, maxDedupeSearch uint64) error {
	...
	for k, v := c.Last(); k != nil; k, v = c.Prev() {
		if bytes.Equal(v, input) {
			return nil // found a duplicate
		}
		if maxDedupeSearch > 0 {
			maxDedupeSearch--
		} else {
			break
		}
...
func trimLength(b *bolt.Bucket, maxItems uint64) error {
	c := b.Cursor()
	// Instead of using Count, we can track items deleted
	deleted := 0
	for k, _ := c.First(); k != nil && deleted < int(maxItems); k, _ = c.First() {
...
func preview(id uint64, v []byte, previewWidth uint) string {
	preview := string(v)
	if len(preview) > int(previewWidth) {
		preview = preview[:int(previewWidth)] + "..."
	}
	return fmt.Sprintf("%d: %s", id, preview)
}
